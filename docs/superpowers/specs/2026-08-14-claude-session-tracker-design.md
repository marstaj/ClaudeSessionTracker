...
hardening, cache format version)
**Status:** Implemented
...
- Extract user and assistant message text from the session's jsonl — one pass
  ...
  `Earlier context (compacted summary): …` with their instruction-shaped
  boilerplate stripped ("this session is being continued…", "if you need
  specific details… read the full transcript at…", "continue the conversation
  from where it left off…"), since left intact it hijacks the recap model.
  Tail cuts only fire near the end of the summary, so a summary quoting one
  of those phrases mid-body keeps its content.
- Spawn hermetic headless Claude:
  `claude -p --model haiku --tools '' --strict-mcp-config --setting-sources "" --output-format json`
...
- Injection hardening: the piped payload brackets the conversation in fence
  markers derived from a per-call random nonce (a fixed fence could be closed
  early by transcript text quoting it), declared as data-not-instructions in
  the prompt, with the recap task repeated after the closing marker so the
  model's last-read instruction is ours. The child is also spawned tool-less
  (`--tools ''`), so even a compliant-but-hijacked model cannot read files or
  act — the prompt defenses raise the bar; the tool-less spawn is the hard
  stop, leaving a wrong recap as the residual risk.
- Cache the recap in `.cache/recaps/<sessionId>.json` keyed by the log file's
  mtime plus a payload format version (`v`) — re-clicking an unchanged session
  returns instantly; a session that grew, or an entry generated by an older
  prompt/extraction format, regenerates. A regenerate button (↻) forces a
  fresh recap via `?force=1`, bypassing the cache.
