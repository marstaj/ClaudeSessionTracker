...
import { codexTurn } from './codex.js';
...
export async function extractConversation(filePath, maxChars = 50_000) {
  ...
  for await (const line of rl) {
    ...
    // Codex rollouts carry their turns as `response_item` records, a type
    // Claude transcripts never use — so one pass reads either format.
    if (obj.type === 'response_item') {
      const turn = codexTurn(obj);
      if (turn) parts.push(`${turn.role === 'user' ? 'User' : 'Assistant'}: ${turn.text}`);
      continue;
    }
